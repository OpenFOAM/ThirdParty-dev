...
export JOBNAME="${JOB_INT}${JOB_THREAD}${JOB_DETERMINISTIC}${JOB_DEBUG}${JOB_RENAME}"
echo ${JOBNAME}
...
case "${JOB_DETERMINISTIC}" in
     n )
       export JOBBUILDFLAGS="${JOBBUILDFLAGS}" ;;
     d )
       export JOBBUILDFLAGS="${JOBBUILDFLAGS} -DSCOTCH_DETERMINISTIC" ;;
esac
